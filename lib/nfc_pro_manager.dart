...
class NfcPro {
  ...
  static void enableDebug(bool enable) {
    // Debug mode for future use
  }

  /// Check if the current device supports NFC and HCE.
  static Future<NfcSupport> checkSupport() async {
    final bool available =
        await _methodChannel.invokeMethod('isAvailable') ?? false;
    final bool hce =
        await _methodChannel.invokeMethod('supportsEmulation') ?? false;
  ...
  static Future<void> startSession({
    ...
    if (_state == NfcSessionState.starting ||
    ...
    try {
      ...
      if (timeout != null) {
        _sessionTimer = Timer(timeout, () async {
          await stopSession();
          if (onError != null) {
            onError(NfcException("Session timed out",
                type: NfcErrorType.timeout, code: "TIMEOUT"));
          }
        });
      }
...
class NfcException implements Exception {
  ...
  NfcException(this.message,
      {this.type = NfcErrorType.unknown, this.code = 'NFC_ERROR'});

  factory NfcException.fromPlatformException(PlatformException e) {
    NfcErrorType type;
    switch (e.code) {
      case 'NOT_SUPPORTED':
        type = NfcErrorType.notSupported;
        break;
      case 'DISABLED':
        type = NfcErrorType.disabled;
        break;
      case 'TIMEOUT':
        type = NfcErrorType.timeout;
        break;
      case 'CONNECTION_LOST':
        type = NfcErrorType.connectionLost;
        break;
      case 'INVALID_APDU':
        type = NfcErrorType.invalidApdu;
        break;
      case 'TAG_LOST':
        type = NfcErrorType.connectionLost;
        break;
